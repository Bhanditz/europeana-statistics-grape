...
module CQ
  ...
  def self.get_account_id_from_slug(account_slug)
    ...
    begin
      account_id = execute_query(@@rumi_io_db_identifier, account_id_query, query_params)
  ...
  def self.get_project_id_from_slug(project_slug, account_id)
    ...
    begin
      project_id = execute_query(@@rumi_io_db_identifier, project_id_query, query_params)
  ...
  def self.get_table_name_from_slug(project_id, file_slug)
    ...
    begin
      table_name = execute_query(@@rumi_io_db_identifier, table_name_query, query_params)
  ...
  def self.get_db_connection_id(project_id, filename)
    ...
    begin
      db_connection = execute_query(@@rumi_io_db_identifier, db_connection_query, query_params)
  ...
  def self.get_grid_table_name(username, projectname, filename)
    account_id = get_account_id_from_slug(username)
    if account_id
      project_id = get_project_id_from_slug(projectname, account_id)
    else
      return false
    end
    if project_id
      table_name = get_table_name_from_slug(project_id, filename)
    else
      return false
    end
    table_name
  ...
  def self.get_human_from_token(project_id, token)
    ...
    begin
      token_validation_resp = execute_query(@@rumi_io_db_identifier, query, query_params)
  ...
  def self.authenticate_token(_account_id, project_id, user_id)
    ...
    begin
      token_validation_resp = execute_query(@@rumi_io_db_identifier, authenticate_token_query, query_params)
      return token_validation_resp.values[0][0]
    rescue
      return false
    end
  ...
  def self.execute_query(db_identifier, query, params)
    unless @@db_pools.key? db_identifier
      @@db_pools[db_identifier] = ConnPool.get_db_connection_pool(db_identifier, 5)
      return false unless @@db_pools[db_identifier]
    end
  ...
  def self.execute_custom_transaction(core_db_connection_id, query, query_params)
    ...
    data = execute_query(@@rumi_io_db_identifier, core_db_connection_query, [core_db_connection_id])
    data = data.first
    begin
      case data['adapter']
      when 'postgresql'
      ...
      name = SecureRandom.hex(5)
      conn.prepare(name, query)
      is_success = if conn.exec_prepared(name, query_params)
                     true
                   else
                     false
                   end
  ...
  def self.execute_custom_query(core_db_connection_id, query, query_params)
    ...
    data = execute_query(@@rumi_io_db_identifier, core_db_connection_query, params)
    data = data.first
    begin
      case data['adapter']
      when 'postgresql'
