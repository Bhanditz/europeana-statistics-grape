...
class Grid
    ...
    if !first_row_header
      headers = []
       ...
       headers.each_with_index do |column_name, pos|
         provided_headers << if column_name
           column_name.dup # dup and clone doesn't do a deep copy.
         else
           "" # dup fails on nil value for column name
                             end

         column_name = get_sql_compatible_column_name(column_name)
     ...
     Grid.create_table(core_db_connection_id, table_name, grid_data[0], provided_headers) unless grid_data.empty? && table_name.is_empty?
  ...
  def self.get_sql_compatible_column_name(column_name)
    ...
    column_name[0] = '_' + column_name[0] unless allowed_ordinal_range.include?column_name[0].ord
    ...
    column_name.each_char.with_index do |char, i|
      unless allowed_ordinal_range.include?char.ord
        column_name[i] = '_'
      end
    end
    column_name
  ...
  def self.create_table(core_db_connection_id, table_name, headers, _provided_headers)
    create_table_query = "CREATE TABLE #{table_name} ("
    create_table_query += ' id SERIAL PRIMARY KEY,'
    headers.each_with_index do |column_name, _pos|
  ...
  def self.insert_values(core_db_connection_id, table_name, grid_data)
    ...
    grid_data.each do |row|
      ...
      next unless row.length == headers.length
      insert_rows_query += '('
      row.each do |value|
