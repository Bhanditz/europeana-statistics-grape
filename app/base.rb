...
class Base < Grape::API
  ...
  helpers do
    def authenticate!(username, projectname, filename, token)
      ...
      if auth_resp[:core_db_connection_id] && auth_resp[:table_name]
        ...
        route_param :datacast_slug do
          ...
          resource :grid do
            ...
            post :create do
              ...
              unless Grid.create(core_db_connection_id, table_name, params[:data], params[:first_row_header])
            ...
            post :delete do
              core_db_connection_id, table_name = authenticate!(params[:account_slug], params[:project_slug], params[:datacast_slug], params[:token])
              unless Grid.delete(core_db_connection_id, table_name)
                time_to_run = Time.now - @girish_start
          ...
          resource :row do
            ...
            post :batch_add do
              core_db_connection_id, table_name = authenticate!(params[:account_slug], params[:project_slug], params[:datacast_slug], params[:token])
              unless Row.batch_add(core_db_connection_id, table_name, params[:data])
                time_to_run = Time.now - @girish_start
